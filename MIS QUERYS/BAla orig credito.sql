...
SELECT case RMDTYPAL
		...
		WHERE rm.CPRCSTNM='' AND DOCDATE <= '20121231' AND VOIDSTTS=0 AND CURTRXAM<>0
		AND r1.CUSTCLAS<>'VENDEDOR' --AND RMDTYPAL=1 
		AND RTRIM(CONVERT(varchar(50),BCHSOURC))+''+RTRIM(CONVERT(varchar(3),RMDTYPAL)) <> 'Sales Entry9'
		GROUP BY RMDTYPAL
		
		SELECT * FROM RM00101
		WHERE CUSTCLAS='CORPORACION' AND CPRCSTNM=''
		
SELECT * FROM RM20201
WHERE  DATE1 BETWEEN '20120101' AND '20121231'      
