...
CM.CUSTNAME Nombre_cliente,
 case
when  RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE, @fecha) <31    THEN SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))
when  RM.RMDTYPAL > 6 AND RM.AGNGBUKT=1 then SUM(RM.CURTRXAM) *-1
else 0
end [menor_30],
 
case
when RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE,@fecha) between 31 and 60 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))
 
else 0
end [31_a_60_Dias],
 
 case
when RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE, @fecha) between 61 and 90 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))

else 0
end [61_a_90_Dias],
 
 case
when RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE, @fecha) between 91 and 120 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))

else 0
end [91_a_120_Dias],
case
when RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE, @fecha) between 121 and 150 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))
else 0
end [121_a_150_Dias],

 case
when RM.RMDTYPAL < 7 AND DATEDIFF(d, RM.DOCDATE, @fecha) between 151 and 180 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))
else 0
end [151_a_180_Dias],

case
when RM.RMDTYPAL <7  AND DATEDIFF(d, RM.DOCDATE, @fecha)>181 then SUM(RM.ORTRXAMT)-SUM(ISNULL(A.MONTO,0))
else 0
end [181_y_mas],
...
WHERE RM.DOCDATE<=@fecha 
AND VOIDSTTS=0 
AND CM.CUSTCLAS IN (@clase)  AND RM.CUSTNMBR='31509'

group by CM.CPRCSTNM,CM.CUSTNMBR, CM.CUSTNAME, CM.PYMTRMID, CM.CUSTCLAS, 
         CM.CRLMTAMT, CS.LASTPYDT,CS.LPYMTAMT,CN.CRDTMGR,CM.SALSTERR, RM.RMDTYPAL,RM.DOCDATE,RM.AGNGBUKT
