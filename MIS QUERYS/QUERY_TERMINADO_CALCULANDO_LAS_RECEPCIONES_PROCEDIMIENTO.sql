...
ALTER procedure PS_REPORTE_HISTORICAL_STATUS_ITEM
@CLASE VARCHAR(200),
...
,ITEMDESC AS DESCRIPCION,IV00101.SELNGUOM AS [UNIDAD DE MEDIDA] ,@LOCALIDAD AS LOCALIDAD ,IV00101.CURRCOST AS 
...
QTYONHND AS CANTIDAD,'' AS DOCUMENT_NUMBER ,'HEADER' AS IDENTIFICAR from IV00101 JOIN  iv00102 ON IV00101.ITEMNMBR=IV00102.ITEMNMBR
 where IV00101.ITMCLSCD IN (@CLASE)--('HT','IB','DETERGENTE','LICORES','IC','IR','IL','ME','MG','MP') 
  and  IV00102.LOCNCODE IN (@LOCALIDAD)
 
 union all
 
 select IV00101.ITEMNMBR AS [NUMERO DE ITEM],IV00101.ITMCLSCD 
 AS CLASE,ITEMDESC AS DESCRIPCION ,IV00101.SELNGUOM 
 AS [UNIDAD DE MEDIDA],@LOCALIDAD AS LOCALIDAD,IV00101.CURRCOST 
 ...
 FROM GPHN.dbo.IV30300 (nolock) LEFT OUTER JOIN GPHN.dbo.IV00101 (nolock)  ON GPHN.dbo.IV30300.ITEMNMBR = GPHN.dbo.IV00101.ITEMNMBR  LEFT OUTER JOIN GPHN.dbo.IV30200 (nolock) 
  ON GPHN.dbo.IV30300.DOCTYPE = GPHN.dbo.IV30200.IVDOCTYP  AND 
  GPHN.dbo.IV30300.DOCNUMBR = GPHN.dbo.IV30200.DOCNUMBR  
  LEFT OUTER JOIN GPHN.dbo.IV40201 (nolock)  ON GPHN.dbo.IV00101.UOMSCHDL = GPHN.dbo.IV40201.UOMSCHDL 
 
   WHERE ((UPPER(ISNULL(GPHN.dbo.iv00101.ITMCLSCD,'')) IN (@CLASE)) --('HT','IB','DETERGENTE','LICORES','IC','IR','IL','ME','MG','MP')) 
   AND (GPHN.dbo.IV30300.DOCDATE > CONVERT(datetime,@FECHA)) AND 
   (UPPER(ISNULL(GPHN.dbo.IV30300.TRXLOCTN,'')) IN (@LOCALIDAD))) 
	and IV30300.DOCTYPE not in (3,4)
...
union all 
 ...
 AS [UNIDAD DE MEDIDA],@LOCALIDAD AS LOCALIDAD,IV00101.CURRCOST 
 AS COSTO_ACTUAL,
 CASE WHEN TRNSTLOC=@LOCALIDAD THEN 
...
AS 
 ...
 FROM GPHN.dbo.IV30300 (nolock) LEFT OUTER JOIN GPHN.dbo.IV00101 (nolock)  ON GPHN.dbo.IV30300.ITEMNMBR = GPHN.dbo.IV00101.ITEMNMBR  
 LEFT OUTER JOIN GPHN.dbo.IV30200 (nolock) 
  ON GPHN.dbo.IV30300.DOCTYPE = GPHN.dbo.IV30200.IVDOCTYP  AND GPHN.dbo.IV30300.DOCNUMBR = GPHN.dbo.IV30200.DOCNUMBR  
  LEFT OUTER JOIN GPHN.dbo.IV40201 (nolock)  ON GPHN.dbo.IV00101.UOMSCHDL = GPHN.dbo.IV40201.UOMSCHDL 
  
   WHERE ((UPPER(ISNULL(GPHN.dbo.iv00101.ITMCLSCD,'')) IN (@CLASE)) -- ('HT','IB','DETERGENTE','LICORES','IC','IR','IL','ME','MG','MP')) 
   AND (GPHN.dbo.IV30300.DOCDATE > CONVERT(datetime,@FECHA)) AND 
   (UPPER(ISNULL(GPHN.dbo.IV30300.TRXLOCTN,'')) IN (@LOCALIDAD) OR IV30300.TRNSTLOC IN (@LOCALIDAD))) 
...
union all
 ...
 AS CLASE,ITEMDESC AS DESCRIPCION ,IV00101.SELNGUOM 
 AS [UNIDAD DE MEDIDA],@LOCALIDAD AS LOCALIDAD,IV00101.CURRCOST 
 AS COSTO_ACTUAL,((IV30300.TRXQTY*POP10500.UMQTYINB)*-1)*IV00101.CURRCOST 
 AS [PRECIO EXTENDIDO] ,(TRXQTY*POP10500.UMQTYINB)*-1 AS 
 CANTIDAD,IV30300.DOCNUMBR AS DOCUMENT_NUMBER ,'' AS IDENTIFICAR
 FROM GPHN.dbo.IV30300 (nolock) LEFT OUTER JOIN GPHN.dbo.IV00101 (nolock)  ON GPHN.dbo.IV30300.ITEMNMBR = GPHN.dbo.IV00101.ITEMNMBR  LEFT OUTER JOIN GPHN.dbo.IV30200 (nolock) 
 ...
 left join POP10500 on IV30300.ITEMNMBR=POP10500.ITEMNMBR AND IV30300.DOCNUMBR=POP10500.POPRCTNM
   WHERE ((UPPER(ISNULL(GPHN.dbo.iv00101.ITMCLSCD,'')) IN (@CLASE))--('HT','IB','DETERGENTE','LICORES','IC','IR','IL','ME','MG','MP')) 
   AND (GPHN.dbo.IV30300.DOCDATE > CONVERT(datetime,@FECHA)) AND 
   (UPPER(ISNULL(GPHN.dbo.IV30300.TRXLOCTN,'')) IN (@LOCALIDAD))) 
	and IV30300.DOCTYPE=4
