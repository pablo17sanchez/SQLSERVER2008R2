...
SELECT     CASE WHEN pm1.VNDCLSID ='INTERNACIO' THEN 'INTERNACIONAL'
         ...
         --  pm33.APTODCDT'Fecha Documento',
           ISNULL(pmInicial.SALDO_INICIAL,0)'SALDO_INICIAL',
           ISNULL(pmFinal.SALDO_FINAL,00)'SALDO_INICIAL'
FROM (
	 SELECT * FROM 
	 ( SELECT  
	         pm2.VENDORID,
...
WHERE VOIDED=0 AND DOCDATE BETWEEN '20120101' AND '20121231'
					...
					SELECT VENDORID,
						 ...
						 (SUM(ISNULL(CASE WHEN pm3.DOCTYPE=1 THEN DOCAMNT END,0))+ SUM(ISNULL(CASE WHEN pm3.DOCTYPE=2 THEN DOCAMNT END,0))+SUM(ISNULL(CASE WHEN pm3.DOCTYPE=3 THEN DOCAMNT END,0)))
						     - (SUM(ISNULL(CASE WHEN pm3.DOCTYPE=4 THEN DOCAMNT END,0))+SUM(ISNULL(CASE WHEN pm3.DOCTYPE=5 THEN DOCAMNT END,0))+SUM(ISNULL(CASE WHEN pm3.DOCTYPE=6 THEN DOCAMNT END,0)))'SALDO_INICIAL'

						FROM GPHN.dbo.PM30200 pm3
...
SELECT * FROM PM00202
WHERE VENDORID='0028' AND YEAR1=2012 AND HISTTYPE=1
